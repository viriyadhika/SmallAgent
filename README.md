...
- Reranking
    ...
    To study the impact of reranking on RAG performance, we designed a simple experiment using a 15-question dataset generated by ChatGPT 5.2 based on the Harry Potter book. Each query was issued to a RAG system with and without a reranking component. The retrieved results were then evaluated by an LLM-as-a-judge (ChatGPT 5.2), which assigned relevance scores from 1 to 5 to assess the quality of each retrieval.
    
    | | No Reranking | Reranking |
    | --- | --- | --- |
    | LLM-as-a-judge Score | 2.9 | 4.3 |


- Reading Tables and Images

    Reading tables is challenging because their structure depends heavily on layout information, which makes computer vision techniques necessary. In our approach, we use YOLO v11 fine-tuned on DocLayNet to detect figures and tables, followed by Table Transformer (TATR) to process table structures. A fine-tuned RoBERTa-based classifier is then applied to determine whether the retrieved text labels are likely to be captions. This captions then can be put into RAG instead of the images themselves, making it to be more useful to be retrieved.
    
    The complete workflow is illustrated below. To construct the RoBERTa fine-tuning dataset, caption examples were collected from news articles, financial statements, and research papers, and then augmented using GPT-5.2. This strategy allows us to use a smaller, significantly faster model instead of a multimodal question-answering LLM, while still achieving strong caption classification performance (approximately 80% accuracy on research papers and financial documents). We expect that further improvements in dataset curation will lead to even higher performance.
    <p align="center">
      <img src="output/Chart2.png" alt="Flow Diagram" height="150">
    </p>

    Below are representative captioning examples. The model successfully classifies long-range captions that are not positioned directly beneath their associated images or tables.
    <table align="center">
      <tr>
        <td>
        <div>
        <img src="output/pdf/success_1_finance.png" width="250">
        <div>Alphabet Financial Report</div>
        </div>
        </td>
        <td>
        <img src="output/pdf/success_2_finance.png" width="250">
        <div>Chipotle financial report</div>
        </td>
      </tr>
      <tr>
        <td>
        <img src="output/pdf/success_3_paper.png" width="250">
        <div>Diffusion Policy - Research Paper</div>
        </td>
        <td>
        <img src="output/pdf/success_4_paper.png" width="250">
        <div>Diffusion Policy - Research Paper</div>
        </td>
      </tr>
    </table>

    Despite these strengths, the model still exhibits the following **failure modes**:
    | Failure mode | Image |
    | --- | --- |
    | Tables often appear visually identical (e.g., when comparing performance metrics across two models), making correct caption pairing difficult | ![Fail 1](output/pdf/failure_1_similar.png) |
    | Cross-attention rerankers lack explicit reasoning capability, limiting their ability to correctly associate images with ambiguous caption candidates | ![Fail 2](output/pdf/failure2_not_enough_info.png) |
...
The system is also tested on embedded table where the table is embedded inside PDF
